...
curl -sL -o ./distros/sandbox/cert-manager/cert-manager.yaml "https://github.com/cert-manager/cert-manager/releases/download/$CERT_MANAGER_VERSION/cert-manager.yaml"
...
"$CLUSTERCMD_PATH" generate provider --core cluster-api --write-to ./infra/capi/cluster-capi/cluster-api-core.yaml
"$CLUSTERCMD_PATH" generate provider --bootstrap kubeadm --write-to ./infra/capi/cluster-capi/cluster-api-bootstrap.yaml
"$CLUSTERCMD_PATH" generate provider --control-plane kubeadm --write-to ./infra/capi/cluster-capi/cluster-api-control-plane.yaml
"$CLUSTERCMD_PATH" generate provider --infrastructure docker --write-to ./infra/capi/cluster-capi-infrastructure-docker/cluster-api-infrastructure-docker.yaml
